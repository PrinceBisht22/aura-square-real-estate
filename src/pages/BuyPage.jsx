...
const BuyPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // Since getResidentialListings may fetch ALL residential (buy and rent), 
        // we filter to just 'Buy' listings here to match the old logic before saving state.
        const buyData = data.filter(item => item.listedFor === 'Buy');
        setAllListings(buyData);
      } catch (err) {
        console.error("Error fetching buy listings:", err);
        setError("Failed to load property listings. Please try refreshing.");
      } finally {
        setLoading(false);
      }
    };
  ...
  const filteredListings = useMemo(() => {
    return allListings.filter((listing) => {
      // 💡 NOTE: The check 'listing.listedFor !== 'Buy'' is mostly redundant if the 
      // API call is correct, but kept for safety.
      // if (listing.listedFor !== 'Buy') return false; 
      ...
      // Price Filter (Uses listing.price)
  ...
  if (loading) {
      return (
          <div className="flex h-[60vh] items-center justify-center">
              ...
              <p className='ml-3 text-lg text-slate-600'>Loading properties for sale...</p>
          </div>
      );
  }
  ...
  return (
    <div className="site-container py-12">
      <header className="space-y-3 pb-8">
        <p className="floating-chip text-navy">Buy</p>
        <h1 className="text-4xl font-display font-semibold text-navy">
          Residential properties for sale
        </h1>
        <p className="text-slate">
          Browse RERA-registered apartments, villas, and studio homes across India with instant
          filters for budget, amenities, and preferred developers.
        </p>
      </header>
        ...
        <section className="space-y-6">
          <div className="rounded-3xl border border-slate/10 bg-white/90 p-4 shadow-card">
            ...
            <p className="text-2xl font-semibold text-navy">
              {filteredListings.length} listings • Page {page} of {totalPages}
            </p>
          </div>
